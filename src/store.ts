...
export class FileStore implements IStore {
	private static readonly DefaultPath = path.join(homedir(), '.ruigem');
...
export class KeytarStore implements IStore {
	static async open(serviceName = 'ruig-ruigem'): Promise<KeytarStore> {
...
export async function verifyPat(pat: string, publisherName?: string): Promise<void> {
	...
	if (!publisherName) {
		try {
			...
			throw new Error(
				`Can not read the publisher's name. Either supply it as an argument or run ruigem from the extension folder. Additional information:\n\n${error}`
			);
		}
	}
...
async function openDefaultStore(): Promise<IStore> {
	if (/^file$/i.test(process.env['ruigem_STORE'] ?? '')) {
